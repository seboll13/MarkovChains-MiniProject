...
if __name__ == "__main__":
    ...
    NUM_QUEENS = 35
    BETA_STAR = 65.8 #beta_star for 35 queens
    #BETA_STAR = 100.6 #beta_star for 50 queens
    ...
    betas = []
    for _beta in range(ceil(BETA_STAR)):
        betas.append(_beta)
    ...
    for _beta in betas:
        ...
        while(j < M):
            #print("Trial",j)
            board = NQueens(beta = _beta, N=NUM_QUEENS)
        ...
        print("Num queens =",NUM_QUEENS,"beta =", _beta, "total number of solutions =", num_solutions)
    
    stringa = 'Total number of solutions for '+str(NUM_QUEENS)+' queens = '+str(num_solutions_list[len(num_solutions_list)-1])
    print(stringa)
    plt.plot(betas,np.log(num_solutions_list),'--ro',label = 'experimental values')
    plt.xlabel(r"$\beta_{t}$")
    plt.ylabel(r"$\log(Z_{\beta_{t}})$")
    plt.legend()
